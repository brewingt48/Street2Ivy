...
import * as projectApplicationProcess from './transactionProcessProjectApplication';
...
export const PROJECT_APPLICATION_PROCESS_NAME = 'default-project-application';
...
const PROCESSES = [
  ...
  {
    name: PROJECT_APPLICATION_PROCESS_NAME,
    alias: `${PROJECT_APPLICATION_PROCESS_NAME}/release-1`,
    process: projectApplicationProcess,
    unitTypes: [INQUIRY],
  },
];
...
export const resolveLatestProcessName = processName => {
  switch (processName) {
    ...
    case PROJECT_APPLICATION_PROCESS_NAME:
      return PROJECT_APPLICATION_PROCESS_NAME;
    default:
      return processName;
  }
};
...
/**
 * Check if the process is project application process
 *
 * @param {String} processName
 */
export const isProjectApplicationProcess = processName => {
  const latestProcessName = resolveLatestProcessName(processName);
  const processInfo = PROCESSES.find(process => process.name === latestProcessName);
  return [PROJECT_APPLICATION_PROCESS_NAME].includes(processInfo?.name);
};

/**
 * Check if the process/alias points to a project application process
 *
 * @param {String} processAlias
 */
export const isProjectApplicationProcessAlias = processAlias => {
  const processName = processAlias ? processAlias.split('/')[0] : null;
  return processAlias ? isProjectApplicationProcess(processName) : false;
};
