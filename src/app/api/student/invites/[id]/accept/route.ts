/**
 * POST /api/student/invites/:id/accept — Accept a corporate invitation
 */
...
export async function POST(
  ...
  try {
    ...
    if (invite.status !== 'pending') {
      return NextResponse.json({ error: 'Can only accept pending invites' }, { status: 400 });
    }
    ...
    await sql`
      ...
      VALUES (
        ...
        'invite_accepted',
        'Invitation Accepted',
        ${studentName + ' accepted your invitation' + (invite.project_title ? ' for "' + invite.project_title + '"' : '')},
