/**
 * GET /api/huddle/bookmarks — List the current user's bookmarked posts
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const countResult = await sql`
      ...
      FROM huddle_bookmarks b
      JOIN huddle_posts p ON p.id = b.post_id
    ...
    const posts = await sql`
      SELECT
        ...
        (SELECT COUNT(*)::int FROM huddle_bookmarks bk WHERE bk.post_id = p.id) AS bookmark_count,
    ...
    console.error('Huddle bookmarks list error:', error);
