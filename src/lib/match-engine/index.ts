...
async function loadListingData(listingId: string): Promise<ListingData | null> {
  ...
  return {
    ...
    startDate: row.start_date ? new Date(row.start_date as string | Date).toISOString() : null,
    endDate: row.end_date ? new Date(row.end_date as string | Date).toISOString() : null,
